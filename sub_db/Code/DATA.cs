...
namespace sub_db
{
    internal class DATA
    {
        ...
        {
            // 番剧名称
            internal string m_name_chs = string.Empty;                  // (简体)
            internal string m_name_cht = string.Empty;                  // (繁体)
            internal string m_name_jp = string.Empty;                   // (日文)
            internal string m_name_en = string.Empty;                   // (英文)
            internal string m_name_rome = string.Empty;                 // (罗马字)

            internal DateTime m_time = DateTime.MinValue;   // 最早放送日期
            internal string m_type = string.Empty;                  // 分类（animation/movie/...）
            internal string m_source = string.Empty;                    // 字幕匹配的片源（DB/DVD/TV/test/...）
            internal string m_sub_name = string.Empty;                  // 字幕名称
            internal string m_extension = string.Empty;                 // 字幕后缀名（多种格式时用;分割，例如：.ass;.srt）
            internal string m_providers = string.Empty;                 // 字幕组/提供者
            internal string m_desc = string.Empty;                  // 字幕说明

            internal string m_path = string.Empty;                  // 路径
        };
